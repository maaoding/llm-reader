...
export function SemanticIndexControls({ bookId, state }: { bookId: string; state: SemanticIndexState }) {
  ...
  const run = async (operation: 'start' | 'rebuild' | 'cancel'): Promise<void> => {
    if (operation === 'rebuild' && !window.confirm(copy('preparation.semanticRebuildConfirm'))) return
  ...
  return <details open className="analysis-details semantic-details" data-testid="semantic-details">
    ...
    {state.status === 'disabled' ? <p className="analysis-hint">{copy('knowledge.embeddingRequired')}</p> : <>
      ...
      {state.total > 0 && <p>{copy('knowledge.indexProgress', { completed: state.completed, total: state.total })} · {state.model}</p>}
