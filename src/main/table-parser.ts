...
function descendants(node: Node, name: string, depth = 0): Node[] {
  if (depth > 64) throw new Error(copy('validation.tableDepth'))
...
function span(node: Node, name: string): number {
  ...
  if (!/^[1-9]\d{0,4}$/u.test(value)) throw new Error(copy('validation.tableSpan'))
  return Number(value)
}
...
export function parseHtmlTable(html: string, id: string): { text: string; table?: DocumentTable; degraded: boolean } {
  ...
  try {
    ...
    if (tables.length !== 1 || descendants(tables[0], 'table').length) throw new Error(copy('validation.tableCount'))
    if (!('sourceCodeLocation' in tables[0]) || !tables[0].sourceCodeLocation || !('endTag' in tables[0].sourceCodeLocation) || !tables[0].sourceCodeLocation.endTag) throw new Error(copy('validation.tableStructure'))
    const rows = descendants(tables[0], 'tr')
    const cells: TableCell[] = [], occupied = new Set<string>()
    const rowTexts: string[][] = []
    if (!rows.length || rows.length > 100_000) throw new Error(copy('validation.tableRows'))
    let columns = 0
    for (const [row, item] of rows.entries()) {
      ...
      for (const child of children(item).filter((node) => ['td', 'th'].includes(tag(node)))) {
        ...
        if (row + rowSpan > rows.length || column + columnSpan > 100_000 || rowSpan * columnSpan > 100_000 || cells.length > 100_000) throw new Error(copy('validation.tableSpan'))
        for (let r = row; r < row + rowSpan; r++) for (let c = column; c < column + columnSpan; c++) {
          const key = `${r}:${c}`
          if (occupied.has(key) || occupied.size > 200_000) throw new Error(copy('validation.tableOverlap'))
          occupied.add(key)
        }
    ...
    if (!cells.length || !columns) throw new Error(copy('validation.tableEmpty'))
