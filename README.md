...
导入无 DRM 的 EPUB 或 UTF-8 TXT，连续滚动阅读并选中原文；划词句段可收藏并在原文持久高亮，阅读区提供独立于界面主题的浅色、米黄、深色纸张主题，并记录可返回的自然阅读位置。应用会把选区与最多 6,000 个 Unicode 字符的章节上下文发送给用户配置的 `/v1/chat/completions` 兼容接口。回答支持流式显示、原文引用跳转、追问与手动收藏。

书籍、自然阅读位置、句段收藏和回答收藏保存在本机。API Key 由 Electron `safeStorage` 加密后写入独立密文文件，不进入 SQLite。
