...
it('migrates old archives and analysis IDs once, preserving timestamps, notes and existing cache identity', async () => {
  ...
  try {
    ...
    database.connection.exec(`ALTER TABLE provider_profiles DROP COLUMN compatibility;
      ...
      ALTER TABLE provider_profiles DROP COLUMN protocol; ALTER TABLE provider_profiles DROP COLUMN request_json; ALTER TABLE provider_profiles DROP COLUMN headers_secret; DELETE FROM schema_migrations WHERE version >= 10;`)
    database.close()
...
it('upgrades a version-10 analysis cache without changing its identity and persists a bounded failure history', async () => {
  ...
  try {
    ...
    database.connection.exec('ALTER TABLE book_analysis DROP COLUMN progress_json; DROP TABLE book_analysis_failures; DROP TABLE book_vectors; DROP TABLE semantic_indexes; DROP TABLE knowledge_settings; DROP TABLE document_jobs; ALTER TABLE provider_profiles DROP COLUMN protocol; ALTER TABLE provider_profiles DROP COLUMN request_json; ALTER TABLE provider_profiles DROP COLUMN headers_secret; DELETE FROM schema_migrations WHERE version >= 11;')
    database.close()
    database = new AppDatabase(path)
