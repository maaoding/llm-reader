...
test('customizes selection action names and prompts and restores them after restart', async () => {
  ...
  try {
    ...
    await page.getByTestId('provider-profile-name').fill('助手操作测试')
    ...
    await page.getByTestId('assistant-explain-icon').selectOption('lightbulb')
    ...
    await page.getByTestId('assistant-context-icon').selectOption('quote')
    await page.getByTestId('assistant-ask-label').fill('直接问')
    await page.getByTestId('assistant-ask-icon').selectOption('pen-line')
    await page.getByTestId('settings-close').click()
    await expect(page.getByTestId('settings-modal')).toHaveCount(0)
    ...
    await expect(page.getByTestId('action-explain')).toHaveAttribute('data-icon', 'lightbulb')
    await expect(page.getByTestId('action-context')).toContainText('看上下文')
    await expect(page.getByTestId('action-context')).toHaveAttribute('data-icon', 'quote')
    await expect(page.getByTestId('action-ask')).toContainText('直接问')
    await expect(page.getByTestId('action-ask')).toHaveAttribute('data-icon', 'pen-line')
    ...
    await expect(restoredPage.getByTestId('action-explain')).toHaveAttribute('data-icon', 'lightbulb')
    ...
    await expect(restoredPage.getByTestId('assistant-explain-icon')).toHaveValue('lightbulb')
    ...
    await expect(restoredPage.getByTestId('assistant-context-icon')).toHaveValue('quote')
    await expect(restoredPage.getByTestId('assistant-ask-label')).toHaveValue('直接问')
    await expect(restoredPage.getByTestId('assistant-ask-icon')).toHaveValue('pen-line')
    ...
    await expect(restoredPage.getByTestId('action-explain')).toHaveAttribute('data-icon', 'highlighter')
    await expect(restoredPage.getByTestId('action-context')).toContainText('联系上下文')
    await expect(restoredPage.getByTestId('action-context')).toHaveAttribute('data-icon', 'book-open')
